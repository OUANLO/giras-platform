...
export function SearchableSelect({
  ...
  const [dropdownPosition, setDropdownPosition] = useState({ top: 0, left: 0, width: 0, mobile: false })
  ...
  // Calculer la position du dropdown quand il s'ouvre ou quand la fenêtre bouge
  useEffect(() => {
    const updateDropdownPosition = () => {
      ...
      setDropdownPosition({
        top: isMobileViewport ? Math.max(16, rect.bottom + 8) : rect.bottom + 4,
        left: isMobileViewport ? 12 : rect.left,
        width: isMobileViewport ? Math.max(window.innerWidth - 24, 220) : rect.width,
  ...
  return (
    <div className={`relative ${className}`} ref={containerRef}>
      ...
      {isOpen && !disabled && typeof document !== 'undefined' && createPortal(
        <div 
          ref={dropdownRef}
          className={`fixed z-[9999] bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden ${dropdownPosition.mobile ? 'max-h-[50vh]' : 'max-h-60'}`}
          style={{
            top: dropdownPosition.top,
            left: dropdownPosition.left,
            width: dropdownPosition.width,
            minWidth: dropdownPosition.mobile ? 'auto' : '200px',
            maxWidth: dropdownPosition.mobile ? 'calc(100vw - 24px)' : 'min(420px, calc(100vw - 24px))'
          }}
        >
...
export function SidebarButton({ icon: Icon, label, active, onClick, badge }) {
  return (
    <button
      onClick={onClick}
      className={`w-full flex items-center gap-3 px-3 sm:px-4 py-3 sm:py-3 rounded-xl text-sm font-medium transition-all duration-300 group border ${
        active
          ? 'bg-gradient-to-r from-[#1a365d] to-[#2c5282] text-white shadow-lg border-transparent'
          : 'text-gray-600 bg-white border-gray-200 hover:bg-gray-50 hover:text-[#1a365d] hover:border-[#1a365d]/20 shadow-sm'
      }`}
    >
...
// ==================== DATA TABLE ====================
export function DataTable({ columns, data, actions, onRowClick, loading, emptyMessage = 'Aucune donnée disponible', compact = false, minWidth = '1200px', actionsWidth = '80px' }) {
  if (loading) {
    return (
        ...
        <table className="w-full text-[10px]" style={{ minWidth: minWidth }}>
          <thead className="bg-gradient-to-r from-[#1a365d] to-[#2c5282]">
            <tr>
              ...
              {actions && (
                <th className="px-2 py-2 text-center text-white sticky right-0 bg-[#2c5282]" style={{ minWidth: actionsWidth, width: actionsWidth }}>
                  Actions
                </th>
              )}
            </tr>
          ...
          <tbody className="divide-y divide-gray-100">
            {data.map((row, rowIndex) => (
              ...
              >
                ...
                {actions && (
                  <td className="px-2 py-1.5 sticky right-0 bg-white" style={{ minWidth: actionsWidth, width: actionsWidth }}>
                    <div className="flex items-center justify-center gap-1">
                      {actions.filter((action) => !(typeof action.hidden === 'function' ? action.hidden(row) : action.hidden)).map((action, i) => (
                        <button
                          key={i}
