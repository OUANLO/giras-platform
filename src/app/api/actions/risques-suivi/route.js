...
export async function GET() {
  try {
    ...
    const [{ data: actions, error: actionsError }, { data: occurrences, error: occError }] = await Promise.all([
      supabase
        .from('actions')
        .select('id, code_action, libelle_action, code_groupe, code_structure, statut_act, code_risque, archive'),
      supabase
        .from('action_occurrences')
        .select('id, code_action, code_occurrence, date_debut, date_fin, tx_avancement, gestionnaire_conf, responsable, archive, date_creation, date_realisation, date_conf')
    ])
    ...
    ;(occurrences || []).forEach((occ) => {
      ...
      rows.push({
        ...
        statut: action.statut_act || 'Actif',
