...
const validateRange = (start, end) => {
  ...
  if (!s || !e) return 'Dates de la première occurrence requises'
  if (e < s) return 'La date de fin de la première occurrence doit être ultérieure ou égale à la date de début'
  return null
}
...
export async function GET(request) {
  try {
    ...
    const actions = (data || []).map(a => {
      const activeOccurrences = (a.action_occurrences || []).filter((occ) => occ && occ.archive !== true)
      const latestOccurrence = [...(activeOccurrences.length > 0 ? activeOccurrences : (a.action_occurrences || []))]
        .sort((x, y) => new Date(y.date_fin || y.date_debut || y.date_creation || 0) - new Date(x.date_fin || x.date_debut || x.date_creation || 0))[0] || null
...
export async function POST(request) {
  try {
    ...
    const { data: existingActionLabels, error: labelCheckError } = await supabase
      .from('actions')
      .select('id, libelle_action')
    if (labelCheckError) {
      return NextResponse.json({ error: labelCheckError.message }, { status: 500 })
    }
    const duplicateAction = findDuplicateByNormalizedLabel(existingActionLabels, 'libelle_action', body.libelle_action)
    if (duplicateAction) {
      return NextResponse.json({ error: duplicateLabelError("d'action", duplicateAction.libelle_action) }, { status: 400 })
    }
    if (!body.code_groupe) {
    ...
    if (body.first_occurrence) {
      const firstOccurrenceDateError = validateRange(body.first_occurrence.date_debut, body.first_occurrence.date_fin)
      if (firstOccurrenceDateError) {
        return NextResponse.json({ error: firstOccurrenceDateError }, { status: 400 })
      }
    ...
    if (projetError) {
      return NextResponse.json({ error: projetError.message }, { status: 500 })
    }
    if (!projet) {
      return NextResponse.json({ error: 'Projet introuvable' }, { status: 404 })
    }
    if (String(projet.statut || 'Actif').trim() !== 'Actif') {
      return NextResponse.json({ error: "Impossible de créer une action pour un projet inactif" }, { status: 400 })
    }
    ...
    // Les dates/plannings sont portés par action_occurrences (pas par actions)
...
export async function PUT(request) {
  try {
    ...
    if (body.libelle_action !== undefined) {
      const { data: existingActionLabels, error: labelCheckError } = await supabase
        .from('actions')
        .select('id, libelle_action')
      if (labelCheckError) {
      ...
      const duplicateAction = findDuplicateByNormalizedLabel(existingActionLabels, 'libelle_action', body.libelle_action, body.id)
      if (duplicateAction) {
        return NextResponse.json({ error: duplicateLabelError("d'action", duplicateAction.libelle_action) }, { status: 400 })
      }
    }
