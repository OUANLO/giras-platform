...
export async function PUT(request) {
  try {
    ...
    // Synchroniser le snapshot risques_probabilites pour la période actuellement ouverte.
    // (Exigence: toute modification d'un risque en "Identification" doit se refléter sur la période ouverte.)
    try {
      const openPeriode = await getOpenPeriode(supabase)
      if (openPeriode) {
        await upsertRisqueProbabiliteSnapshot({
          supabase,
          periode: openPeriode,
          codeRisque: data.code_risque,
          modificateur: body.modificateur,
      ...
      // Ne pas bloquer la sauvegarde du risque si la synchro échoue
      console.error('Erreur sync risques_probabilites (PUT risque):', syncErr)
    }
