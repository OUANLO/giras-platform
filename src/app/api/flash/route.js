...
export async function GET(request) {
  try {
    ...
    if (all === 'true') {
      ...
      if (!canAccessAdminSection(guard, 'flash')) return NextResponse.json({ error: 'Accès refusé' }, { status: 403 })
    }
...
export async function POST(request) {
  try {
    const guard = requireAdministrationAccess(request)
    if (guard instanceof NextResponse) return guard
    if (!canEditAdminSection(guard, 'flash')) return NextResponse.json({ error: 'Accès refusé' }, { status: 403 })
    if (!canEditAdminSection(guard, 'flash')) return NextResponse.json({ error: 'Accès refusé' }, { status: 403 })
    if (!canEditAdminSection(guard, 'flash')) return NextResponse.json({ error: 'Accès refusé' }, { status: 403 })
