...
const validateTaskDates = (start, end) => {
  ...
  if (!s || !e) return 'Dates obligatoires'
  if (e < s) return 'La date de fin doit être ultérieure ou égale à la date de début'
  return null
}


const getOccurrenceForTask = async (supabase, codeOccurrence) => {
  if (!codeOccurrence) return null
  const { data, error } = await supabase
    .from('action_occurrences')
    .select('id, code_occurrence, tx_avancement, validation_status, gestionnaire_conf')
    .eq('code_occurrence', parseInt(codeOccurrence))
    .maybeSingle()
  if (error) throw error
  return data || null
}

const isOccurrenceTaskLocked = (occurrence) => String(occurrence?.validation_status || '').toLowerCase() === 'achevée' && String(occurrence?.gestionnaire_conf || '').toLowerCase() === 'oui'
...
const recomputeAndSyncOccurrenceTx = async (supabase, code_occurrence, modificateur = null) => {
  ...
  try {
    ...
    if (shouldDone) {
      ...
      update.validation_status = String(occ.gestionnaire_conf || '').toLowerCase() === 'oui' ? 'Achevée' : 'Terminée - non confirmée'
    } else {
      ...
      update.validation_status = tx > 50 ? 'En cours +50%' : tx > 0 ? 'En cours -50%' : 'Non entamée'
    }
...
export async function POST(request) {
  try {
    ...
    const dateError = validateTaskDates(body.date_debut, body.date_fin)
    if (dateError) {
    ...
    const parentOccurrence = await getOccurrenceForTask(supabase, body.code_occurrence)
    if (!parentOccurrence?.id) {
      return NextResponse.json({ error: 'Occurrence introuvable' }, { status: 404 })
    }
    if (isOccurrenceTaskLocked(parentOccurrence)) {
      return NextResponse.json({ error: "Cette action est au statut Achevée. Toutes les tâches sont en lecture seule." }, { status: 400 })
    }
...
export async function PUT(request) {
  try {
    ...
    const { data: oldTache } = await supabase
      .from('taches')
      .select('responsable, code_action, libelle_tache, code_occurrence')
      .eq('id', body.id)
      .single()

    const parentOccurrence = await getOccurrenceForTask(supabase, body.code_occurrence || oldTache?.code_occurrence)
    if (isOccurrenceTaskLocked(parentOccurrence)) {
      return NextResponse.json({ error: "Cette action est au statut Achevée. Toutes les tâches sont en lecture seule." }, { status: 400 })
    }
...
export async function DELETE(request) {
  try {
    ...
    const parentOccurrence = await getOccurrenceForTask(supabase, old?.code_occurrence)
    if (isOccurrenceTaskLocked(parentOccurrence)) {
      return NextResponse.json({ error: "Cette action est au statut Achevée. Toutes les tâches sont en lecture seule." }, { status: 400 })
    }
