...
function getSupabaseClient(request) {
  ...
  return createAuditedClient(client, request)
}
  ...
  try {
    ...
    if (action === 'close') {
      ...
      if (fichierCartographie) {
        ...
        const isStringFile = typeof fichierCartographie === 'string'
        const storageBucket = !isStringFile ? (fichierCartographie.storage_bucket || fichierCartographie.bucket || null) : null
        const storagePath = !isStringFile ? (fichierCartographie.storage_path || fichierCartographie.path || null) : null
        const rawValue = isStringFile
          ? fichierCartographie
          : (storageBucket && storagePath ? `storage://${storageBucket}/${storagePath}` : null)
        const uploadedFileName = !isStringFile ? (fichierCartographie.nom_fichier || fichierCartographie.fileName || null) : null
        const basePayload = {
          code_periode: codePeriode,
          libelle_periode: libellePeriode,
          nom_fichier: uploadedFileName || `Cartographie_${String(libellePeriode || codePeriode).replace(/\s+/g, '_')}.pdf`,
          url_fichier: rawValue,
