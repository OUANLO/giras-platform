...
export async function GET() {
  try {
    ...
    const enrichedActions = (actions || []).map((action) => {
      ...
      const activeOccurrences = (action.action_occurrences || []).filter((occ) => occ && occ.archive !== true)
      ...
      return {
        ...
        code_structure_resp: latestOccurrence?.code_structure || action.code_structure
      }
    })
