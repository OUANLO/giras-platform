...
async function validateIndicatorResponsible(supabase, body) {
  const selected = String(body.responsable || '').trim()
  ...
  if (!selectedUser) {
    const err = new Error('Responsable introuvable')
    err.status = 400
    throw err
  }

  if (selectedUser.structure === body.code_structure) return selected

  const groupCodes = Array.isArray(body.groupes)
    ? body.groupes.filter(Boolean).map(v => String(v).trim()).filter(Boolean)
    : []
  ...
  const err = new Error("Le responsable doit être membre de la structure sélectionnée ou gestionnaire du groupe d'indicateurs")
  err.status = 400
  throw err
}
...
export async function GET(request) {
  try {
    ...
    if (!includeArchived) {
      ...
      else query = query.or('archive.is.null,archive.eq.false')
    }
    ...
    if (error) {
      ...
      if (!includeArchived) {
        if (archivedOnly) simpleQuery = simpleQuery.eq('archive', true)
        else simpleQuery = simpleQuery.or('archive.is.null,archive.eq.false')
      }
...
export async function POST(request) {
  try {
    ...
    const { data: existingIndicatorLabels, error: labelCheckError } = await supabase
      .from('indicateurs')
      .select('id, libelle_indicateur')
    if (labelCheckError) {
      return NextResponse.json({ error: labelCheckError.message }, { status: 500 })
    }
    const duplicateIndicator = findDuplicateByNormalizedLabel(existingIndicatorLabels, 'libelle_indicateur', body.libelle_indicateur)
    if (duplicateIndicator) {
      return NextResponse.json({ error: duplicateLabelError("d'indicateur", duplicateIndicator.libelle_indicateur) }, { status: 400 })
    }
    ...
    const selectedGroupCodes = Array.isArray(body.groupes) ? body.groupes.filter(Boolean) : []
    if (selectedGroupCodes.length > 0) {
      const { data: groups, error: groupsError } = await supabase
        .from('groupe_indicateurs')
        .select('code_groupe, statut')
        .in('code_groupe', selectedGroupCodes)
      if (groupsError) {
        return NextResponse.json({ error: groupsError.message }, { status: 500 })
      }
      const inactiveGroup = (groups || []).find((g) => String(g.statut || 'Actif').trim() !== 'Actif')
      if (inactiveGroup) {
        return NextResponse.json({ error: "Impossible de créer un indicateur dans un groupe inactif" }, { status: 400 })
      }
    }

    try {
      await validateIndicatorResponsible(supabase, body)
    } catch (validationError) {
      return NextResponse.json({ error: validationError.message }, { status: validationError.status || 500 })
    }
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        necessite_cible: body.necessite_cible === 'Non' ? 'Non' : 'Oui',
...
export async function PUT(request) {
  try {
    ...
    if (body.libelle_indicateur !== undefined) {
      const { data: existingIndicatorLabels, error: labelCheckError } = await supabase
        .from('indicateurs')
        .select('id, libelle_indicateur')
      if (labelCheckError) {
      ...
      const duplicateIndicator = findDuplicateByNormalizedLabel(existingIndicatorLabels, 'libelle_indicateur', body.libelle_indicateur, body.id)
      if (duplicateIndicator) {
        return NextResponse.json({ error: duplicateLabelError("d'indicateur", duplicateIndicator.libelle_indicateur) }, { status: 400 })
      }
    }
...
export async function DELETE(request) {
  try {
    ...
    if (occDeleteError) {
      console.error('Erreur delete occurrences indicateur:', occDeleteError)
      return NextResponse.json({ error: occDeleteError.message }, { status: 500 })
    }
    ...
    if (risquesProbError && !/column .* does not exist/i.test(risquesProbError.message || '')) {
      console.error('Erreur delete risques_probabilites indicateur:', risquesProbError)
    ...
    return NextResponse.json({ success: true, message: 'Indicateur supprimé avec ses occurrences associées' })
  } catch (error) {
    console.error('Erreur DELETE indicateur:', error)
    return NextResponse.json({ error: error.message }, { status: 500 })
  }
