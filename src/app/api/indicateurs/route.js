...
export async function PUT(request) {
  try {
    ...
    // Synchroniser risques_probabilites pour la période ouverte si cet indicateur est lié à des risques.
    // (Exigence: toute modification d'un indicateur dans "Indicateurs" doit mettre à jour la photo du risque
    //            pour la période ouverte.)
    try {
      const openPeriode = await getOpenPeriode(supabase)
      if (openPeriode && data?.code_indicateur) {
      ...
      console.error('Erreur sync risques_probabilites (PUT indicateur):', syncErr)
    }
