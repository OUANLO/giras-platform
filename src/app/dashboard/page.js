...
import { Shield, Activity, BarChart3, TrendingUp, PieChart, Settings, ArrowRight, AlertTriangle, CheckCircle, Clock3, ClipboardList, PencilLine, FileText } from 'lucide-react'
  ...
  const fetchDashboardSummary = async () => {
    try {
      ...
      const pendingRisksRows = openPeriodAliases.size
        ...
        : []
      const pendingRisks = pendingRisksRows.length
      ...
      <div className="max-w-6xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 mb-5">
          {synthesisCards.map((card) => {
            const isDisabled = !card.access
            return (
            <div key={card.key} className={`rounded-2xl border shadow-sm px-4 py-3 lg:px-4 lg:py-3.5 ${isDisabled ? 'border-gray-200 bg-gray-100/90 opacity-70' : card.accent}`}>
              <div className="flex items-start gap-3">
                <div className={`p-2 rounded-lg shrink-0 ${isDisabled ? 'bg-gray-200 text-gray-400' : card.iconBg}`}>
                  <card.icon size={16} />
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  {card.showButton && !loading && card.access && (
                    <button
                  ...
                  {isDisabled && (
                    <div className="mt-2 text-[11px] font-medium text-gray-400">Rubrique non autorisée</div>
                  )}
                </div>
              </div>
            </div>
