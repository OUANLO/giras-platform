...
import { FolderOpen, BarChart2, TrendingUp, Plus, Edit, Trash2, List, PlayCircle, Eye, CheckCircle, AlertTriangle, XCircle, ChevronLeft, ChevronRight, X, Info, RotateCcw, ChevronDown, Download, Archive } from 'lucide-react'
import { Button, Modal, FormInput, StatusBadge, SidebarButton, AlertModal } from '@/components/ui'
import * as XLSX from 'xlsx'
import { canAccessIndicator, canAccessIndicatorOccurrence, isPrivilegedUser } from '@/lib/access-scope'
...
export default function IndicateursPage() {
  ...
  const fetchArchivedGroupesOptions = async () => {
    try {
      const r = await fetch('/api/archive?type=groupe_indicateurs')
      if (r.ok) setArchivedGroupes((await r.json()) || [])
    } catch {}
  }
  const fetchBaseData = async () => {
    try {
      const [sR, uR, aR] = await Promise.all([fetch('/api/structures'), fetch('/api/users'), fetch('/api/archive?type=groupe_indicateurs')])
  ...
  const fetchGroupes = async () => { setLoading(true); try { const r = await fetch('/api/groupe-indicateurs'); if (r.ok) setGroupes((await r.json()).groupes || []) ; await fetchArchivedGroupesOptions() } catch {} setLoading(false) }
  ...
  const canViewInd = (ind) => isGestionnaireOnIndicator(ind) || canAccessIndicator(user, ind, users)
  const canViewOcc = (occ) => isGestionnaireOnOccurrence(occ) || canAccessIndicatorOccurrence(user, occ, [...indicateurs, ...archivedIndicateurs], users)
  ...
  const availableManagedGroupsForForm = (selectedIndicateur || isAdmin()
    ? groupes
  ...
  const handleDelGrp = (g) => { 
    ...
    setConfirmAction({ message: `Supprimer "${g.libelle_groupe}" ? Cette suppression entraînera aussi la suppression de tous les indicateurs et de toutes les occurrences associés à ce groupe.`, onConfirm: async () => {
      try { const r = await fetch('/api/groupe-indicateurs', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: g.id }) }); if (r.ok) { showAlert('success', 'Groupe, indicateurs et occurrences supprimés', async () => { await fetchGroupes(); await fetchIndicateurs(); await fetchOccurrences(); if (showArchivedGroupesModal) handleOpenArchivedGroupes(); if (showArchivedIndicateursModal) handleOpenArchivedIndicateurs(); if (showArchivedSuiviModal) handleOpenArchivedSuivi() }) } else { const err = await r.json(); showAlert('error', err.error || 'Erreur') } } catch { showAlert('error', 'Erreur') }
    }})
  }
  ...
  const handleDelInd = (ind) => { 
    ...
    setConfirmAction({ message: `Supprimer l'indicateur "${ind.libelle_indicateur}" ? Cette suppression entraînera aussi la suppression de toutes ses occurrences.`, onConfirm: async () => {
      try { const r = await fetch('/api/indicateurs', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: ind.id }) }); if (r.ok) { showAlert('success', 'Indicateur et occurrences supprimés', async () => { await fetchIndicateurs(); await fetchOccurrences(); if (showArchivedIndicateursModal) handleOpenArchivedIndicateurs(); if (showArchivedSuiviModal) handleOpenArchivedSuivi() }) } else { const err = await r.json(); showAlert('error', err.error || 'Erreur') } } catch { showAlert('error', 'Erreur') }
    }})
  }
        ...
        showAlert('success', type === 'groupe_indicateurs' ? 'Groupe, indicateurs et occurrences archivés' : type === 'indicateur' ? 'Indicateur et occurrences archivés' : 'Élément archivé avec succès', async () => {
          if (type === 'groupe_indicateurs') {
            ...
            if (showOccurrencesListModal && selectedIndicateur) {
              const rr = await fetch(`/api/indicateurs/occurrences?code_indicateur=${selectedIndicateur.code_indicateur}`)
  ...
  const filterGroupesList = (list) => list.filter(g => {
    ...
    if (groupeFilters.gestionnaire && !g.gestionnaires?.includes(groupeFilters.gestionnaire) && g.gestionnaire !== groupeFilters.gestionnaire) return false
  ...
  const filterIndicateursList = (list) => list.filter(i => { if (!canViewInd(i)) return false; if (indicateurFilters.structure && i.code_structure !== indicateurFilters.structure) return false; if (indicateurFilters.groupe && !i.groupes?.includes(indicateurFilters.groupe) && i.code_groupe !== indicateurFilters.groupe) return false; if (indicateurFilters.type_indicateur && i.type_indicateur !== indicateurFilters.type_indicateur) return false; if (indicateurFilters.statut && i.statut !== indicateurFilters.statut) return false; if (indicateurFilters.responsable && i.responsable !== indicateurFilters.responsable) return false; if (indicateurFilters.recherche && !i.libelle_indicateur?.toLowerCase().includes(indicateurFilters.recherche.toLowerCase())) return false; return true })
  const filterOccurrencesList = (list) => list.filter(o => { const ind = indicateurs.find(i => i.code_indicateur === o.code_indicateur) || archivedIndicateurs.find(i => i.code_indicateur === o.code_indicateur) || o.indicateur || null; if (!canViewOcc(o)) return false; const indGroupes = ind?.groupes || o.groupes || []; const indCodeGroupe = ind?.code_groupe || o.code_groupe; const indStructure = ind?.code_structure || o.code_structure; const indResponsable = ind?.responsable || o.responsable; const indLibelle = ind?.libelle_indicateur || o.libelle_indicateur; const indSens = ind?.sens || o.sens; if (suiviFilters.groupe && !indGroupes?.includes(suiviFilters.groupe) && indCodeGroupe !== suiviFilters.groupe) return false; if (suiviFilters.structure && indStructure !== suiviFilters.structure) return false; if (suiviFilters.indicateur && o.code_indicateur !== parseInt(suiviFilters.indicateur)) return false; if (suiviFilters.responsable && indResponsable !== suiviFilters.responsable) return false; if (suiviFilters.statut) { const ret = calcRetard(o) || { enRetard: false }; if (suiviFilters.statut === 'Retard' && !ret.enRetard) return false; if (suiviFilters.statut === 'Pas retard' && ret.enRetard) return false } if (suiviFilters.date_debut && o.date_debut < suiviFilters.date_debut) return false; if (suiviFilters.date_fin && o.date_fin > suiviFilters.date_fin) return false; if (suiviFilters.atteinte) { const cibleVal = getCible(o, ind || o); const att = getAtteinte(o.val_indicateur, cibleVal, indSens); if (suiviFilters.atteinte !== att.status) return false } if (suiviFilters.renseignement === 'oui' && o.val_indicateur == null) return false; if (suiviFilters.renseignement === 'non' && o.val_indicateur != null) return false; if (dashboardPendingOnly && o.val_indicateur != null) return false; if (suiviFilters.recherche && !indLibelle?.toLowerCase().includes(suiviFilters.recherche.toLowerCase())) return false; return true })
  ...
  const handleOpenArchivedGroupes = async () => {
    try {
      const rr = await fetch('/api/archive?type=groupe_indicateurs')
  ...
  const handleOpenArchivedIndicateurs = async () => {
    try {
      const rr = await fetch('/api/archive?type=indicateur')
      const jj = rr.ok ? await rr.json() : []
  ...
  const handleOpenArchivedSuivi = async () => {
    try {
      const rr = await fetch('/api/archive?type=suivi_indicateur')
      const jj = rr.ok ? await rr.json() : []
    ...
    <div className="flex h-[calc(100vh-140px)]">
      ...
      <div className="flex-1 min-w-0 overflow-auto p-1">
        {activeTab==='groupes'&&<div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4">
          <div className="flex items-center justify-between mb-4"><h2 className="text-lg font-semibold text-gray-800">Groupes</h2><div className="flex gap-2"><Button size="sm" variant="secondary" onClick={handleOpenArchivedGroupes}><Archive size={14} className="mr-1"/>Archives</Button>{isAdmin()&&<Button size="sm" onClick={()=>handleOpenGrpModal()}><Plus size={14} className="mr-1"/>Nouveau</Button>}<Button size="sm" variant="secondary" onClick={exportGroupesToExcel}><Download size={14} className="mr-1"/>Excel</Button></div></div>
        ...
        {activeTab==='suivi'&&<div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4">
          <div className="flex items-center justify-between mb-4"><h2 className="text-lg font-semibold text-gray-800">Suivi</h2><div className="flex gap-2"><Button size="sm" variant="secondary" onClick={handleOpenArchivedSuivi}><Archive size={14} className="mr-1"/>Archives</Button><Button size="sm" variant="secondary" onClick={exportSuiviToExcel}><Download size={14} className="mr-1"/>Excel</Button></div></div>
          <div className="bg-gray-50 rounded-lg p-3 mb-4">
      ...
      <Modal isOpen={showArchivedIndicateursModal} onClose={()=>setShowArchivedIndicateursModal(false)} title="Archives - Indicateurs" size="xl">
        <div className="bg-gray-50 rounded-lg p-3 mb-4"><div className="flex gap-2 items-end flex-wrap"><div className="w-48"><SearchableSelect label="Structure" value={indicateurFilters.structure} onChange={v=>setIndicateurFilters({...indicateurFilters,structure:v})} options={structOpts} size="sm"/></div><div className="w-48"><SearchableSelect label="Groupe" value={indicateurFilters.groupe} onChange={v=>setIndicateurFilters({...indicateurFilters,groupe:v})} options={grpOpts} size="sm"/></div><div className="w-32"><label className="block text-[10px] font-medium text-gray-500 mb-1">Type</label><select value={indicateurFilters.type_indicateur} onChange={e=>setIndicateurFilters({...indicateurFilters,type_indicateur:e.target.value})} className="w-full px-2 py-1.5 text-xs border rounded"><option value="">Tous</option><option value="Taux">Taux</option><option value="TxCalcule">Tx%</option><option value="Nombre">Nombre</option></select></div><div className="w-28"><label className="block text-[10px] font-medium text-gray-500 mb-1">Statut</label><select value={indicateurFilters.statut} onChange={e=>setIndicateurFilters({...indicateurFilters,statut:e.target.value})} className="w-full px-2 py-1.5 text-xs border rounded"><option value="">Tous</option><option value="Actif">Actif</option><option value="Inactif">Inactif</option></select></div><div className="w-48"><SearchableSelect label="Responsable" value={indicateurFilters.responsable} onChange={v=>setIndicateurFilters({...indicateurFilters,responsable:v})} options={gestOpts} size="sm"/></div><div className="flex-1 min-w-[120px]"><label className="block text-[10px] font-medium text-gray-500 mb-1">Recherche</label><input type="text" placeholder="..." value={indicateurFilters.recherche} onChange={e=>setIndicateurFilters({...indicateurFilters,recherche:e.target.value})} className="w-full px-2 py-1.5 text-xs border rounded"/></div><button onClick={()=>setIndicateurFilters({structure:'',groupe:'',type_indicateur:'',statut:'',responsable:'',recherche:''})} className="p-1.5 hover:bg-gray-100 rounded border"><RotateCcw size={14} className="text-gray-600"/></button></div></div>
      ...
      <Modal isOpen={showArchivedSuiviModal} onClose={()=>setShowArchivedSuiviModal(false)} title="Archives - Suivi" size="xl">
        <div className="bg-gray-50 rounded-lg p-3 mb-4"><div className="flex gap-2 items-end flex-wrap"><div className="w-48"><SearchableSelect label="Groupe" value={suiviFilters.groupe} onChange={v=>setSuiviFilters({...suiviFilters,groupe:v})} options={grpOpts} size="sm"/></div><div className="w-48"><SearchableSelect label="Structure" value={suiviFilters.structure} onChange={v=>setSuiviFilters({...suiviFilters,structure:v})} options={structOpts} size="sm"/></div><div className="w-52"><SearchableSelect label="Indicateur" value={suiviFilters.indicateur} onChange={v=>setSuiviFilters({...suiviFilters,indicateur:v})} options={indOpts} size="sm"/></div><div className="w-48"><SearchableSelect label="Responsable" value={suiviFilters.responsable} onChange={v=>setSuiviFilters({...suiviFilters,responsable:v})} options={gestOpts} size="sm"/></div><div className="w-28"><label className="block text-[10px] font-medium text-gray-500 mb-1">Statut</label><select value={suiviFilters.statut} onChange={e=>setSuiviFilters({...suiviFilters,statut:e.target.value})} className="w-full px-2 py-1.5 text-xs border rounded"><option value="">Tous</option><option value="Retard">Retard</option><option value="Pas retard">Pas retard</option></select></div><div className="w-28"><label className="block text-[10px] font-medium text-gray-500 mb-1">Atteinte</label><select value={suiviFilters.atteinte} onChange={e=>setSuiviFilters({...suiviFilters,atteinte:e.target.value})} className="w-full px-2 py-1.5 text-xs border rounded"><option value="">Toutes</option><option value="atteint">Atteint</option><option value="proche">Proche</option><option value="non_atteint">Non atteint</option></select></div><div className="w-32"><label className="block text-[10px] font-medium text-gray-500 mb-1">Début ≥</label><input type="date" value={suiviFilters.date_debut} onChange={e=>setSuiviFilters({...suiviFilters,date_debut:e.target.value})} className="w-full px-2 py-1.5 text-xs border rounded"/></div><div className="w-32"><label className="block text-[10px] font-medium text-gray-500 mb-1">Fin ≤</label><input type="date" value={suiviFilters.date_fin} onChange={e=>setSuiviFilters({...suiviFilters,date_fin:e.target.value})} className="w-full px-2 py-1.5 text-xs border rounded"/></div><div className="w-32"><label className="block text-[10px] font-medium text-gray-500 mb-1">Renseigné</label><select value={suiviFilters.renseignement} onChange={e=>setSuiviFilters({...suiviFilters,renseignement:e.target.value})} className="w-full px-2 py-1.5 text-xs border rounded"><option value="">Tous</option><option value="oui">Oui</option><option value="non">Non</option></select></div><div className="flex-1 min-w-[120px]"><label className="block text-[10px] font-medium text-gray-500 mb-1">Recherche</label><input type="text" placeholder="..." value={suiviFilters.recherche} onChange={e=>setSuiviFilters({...suiviFilters,recherche:e.target.value})} className="w-full px-2 py-1.5 text-xs border rounded"/></div><button onClick={()=>setSuiviFilters({groupe:'',structure:'',indicateur:'',responsable:'',statut:'',atteinte:'',date_debut:'',date_fin:'',renseignement:'',recherche:''})} className="p-1.5 hover:bg-gray-100 rounded border"><RotateCcw size={14} className="text-gray-600"/></button></div>{dashboardPendingOnly && <div className="mt-2 inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-purple-50 border border-purple-200 text-[11px] text-purple-800">Affichage direct des occurrences d'indicateurs en attente de saisie.</div>}</div>
        <div className="relative"><div className="overflow-x-auto overflow-y-auto" style={{maxHeight:'550px'}}><table className="w-full text-[10px]" style={{minWidth:'900px'}}><thead className="bg-gradient-to-r from-[#1a365d] to-[#2c5282] sticky top-0 z-20"><tr><th className="px-2 py-2 text-left text-white sticky left-0 bg-[#1a365d] z-30" style={{minWidth:'140px'}}>Libellé</th><th className="px-2 py-2 text-center text-white">Période</th><th className="px-2 py-2 text-center text-white">Début</th><th className="px-2 py-2 text-center text-white">Fin</th><th className="px-2 py-2 text-center text-white">Limite</th><th className="px-2 py-2 text-center text-white">Val</th><th className="px-2 py-2 text-center text-white">Cible</th><th className="px-2 py-2 text-center text-white">Att.</th><th className="px-2 py-2 text-center text-white">Saisie</th><th className="px-2 py-2 text-center text-white">Ret.</th><th className="px-2 py-2 text-center text-white">St.</th><th className="px-2 py-2 text-center text-white sticky right-0 bg-[#1a365d] z-30" style={{width:'50px'}}>Act</th></tr></thead><tbody className="divide-y divide-gray-100">{fArchivedOcc.length===0?<tr><td colSpan={12} className="text-center py-8 text-gray-500">Aucune</td></tr>:fArchivedOcc.map(occ=>{const ind=indicateurs.find(i=>i.code_indicateur===occ.code_indicateur) || archivedIndicateurs.find(i=>i.code_indicateur===occ.code_indicateur);const cibleVal=getCible(occ,ind);const att=getAtteinte(occ.val_indicateur,cibleVal,ind?.sens);const ret=calcRetard(occ);const isTx=ind?.type_indicateur==='Taux'||ind?.type_indicateur==='TxCalcule';return<tr key={occ.id} className="hover:bg-gray-50"><td className="px-2 py-1.5 whitespace-nowrap overflow-hidden text-ellipsis sticky left-0 bg-white z-10" style={{maxWidth:'140px'}} title={occ.libelle_indicateur || ind?.libelle_indicateur}>{occ.libelle_indicateur || ind?.libelle_indicateur || '-'}</td><td className="px-2 py-1.5 text-center">{occ.periode||'-'}</td><td className="px-2 py-1.5 text-center">{fmtDate(occ.date_debut)}</td><td className="px-2 py-1.5 text-center">{fmtDate(occ.date_fin)}</td><td className="px-2 py-1.5 text-center">{fmtDate(occ.date_limite_saisie)}</td><td className="px-2 py-1.5 text-center font-medium">{occ.val_indicateur!=null?(isTx?`${parseFloat(occ.val_indicateur).toFixed(1)}%`:occ.val_indicateur):'-'}</td><td className="px-2 py-1.5 text-center">{cibleVal!=null?(isTx?`${cibleVal}%`:cibleVal):'-'}</td><td className="px-2 py-1.5 text-center"><AttBadge a={att}/></td><td className="px-2 py-1.5 text-center">{fmtDate(occ.date_saisie)}</td><td className="px-2 py-1.5 text-center"><RetBadge ret={ret}/></td><td className="px-2 py-1.5 text-center"><StBadge ret={ret}/></td><td className="px-2 py-1.5 text-center sticky right-0 bg-white z-10"><div className="flex justify-center gap-1"><button onClick={()=>handleDeleteArchivedItem('suivi_indicateur', occ, `${ind?.libelle_indicateur||'Occurrence'} - ${occ.periode||'Perso.'}`)} className="p-1 text-red-600 hover:bg-red-100 rounded" title="Supprimer définitivement"><Trash2 size={12}/></button></div></td></tr>})}</tbody></table></div></div>
