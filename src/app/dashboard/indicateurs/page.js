...
export default function IndicateursPage() {
  ...
  const calcRetard = (occ) => { const dateLimite = occ.date_limite_saisie ? new Date(occ.date_limite_saisie) : null; if (!dateLimite) return { jours: 0, enRetard: false }; const dateRef = occ.val_indicateur != null && occ.date_saisie ? new Date(occ.date_saisie) : new Date(); const diffTime = dateRef.getTime() - dateLimite.getTime(); const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); return { jours: diffDays, enRetard: diffDays > 0 } }
    ...
    <div className="flex h-[calc(100vh-140px)]">
      ...
      <div className="flex-1 min-w-0 overflow-auto p-1">
        ...
        {activeTab==='suivi'&&<div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4">
          <div className="flex items-center justify-between mb-4"><h2 className="text-lg font-semibold text-gray-800">Suivi</h2></div>
          <div className="bg-gray-50 rounded-lg p-3 mb-4">
            <div className="flex gap-2 items-end flex-wrap">
              ...
              <Button size="sm" variant="secondary" onClick={exportSuiviToExcel}><Download size={14} className="mr-1"/>Excel</Button>
            </div>
          </div>
