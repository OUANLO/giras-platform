...
import { BarChart3, Target, Activity, CheckCircle, AlertTriangle, TrendingUp, Search, Maximize2, X, Clock, Users, FileText, Percent, Calendar, List, ListChecks, Timer, RotateCcw, Layers, Shield, TrendingDown, Download } from 'lucide-react'
...
export default function TableauBordPage() {
  ...
  const indicatorsExportRef = useRef(null)
  const actionsExportRef = useRef(null)
  const risquesExportRef = useRef(null)
  ...
  const exportDashboardSectionToPdf = async (type) => {
    const target = type === 'indicateurs' ? indicatorsExportRef.current : type === 'actions' ? actionsExportRef.current : risquesExportRef.current
    if (!target) return
    try {
      const html2canvas = (await import('html2canvas')).default
      const sectionTitles = {
        risques: 'Tableau de bord - Suivi des risques',
        indicateurs: 'Tableau de bord - Suivi des indicateurs',
        actions: 'Tableau de bord - Suivi des actions',
      }

      const buildExportWrapper = (contentNode, title) => {
        ...
        heading.style.background = 'linear-gradient(135deg, #1a365d 0%, #2c5282 100%)'
        heading.style.color = '#fff'
        heading.style.padding = '18px 24px'
        heading.style.borderRadius = '18px'
        heading.style.marginBottom = '18px'
        heading.innerHTML = `<div style="font-size:28px;font-weight:700;">${title}</div><div style="font-size:12px;color:#dbeafe;margin-top:6px;">Export généré le ${new Date().toLocaleString('fr-FR')}</div>`
      ...
      const prepareClone = (node) => {
        ...
        clone.querySelectorAll('[data-export-exclude="true"]').forEach((item) => item.remove())
      ...
      const renderNodeToCanvas = async (node, title) => {
        const clone = prepareClone(node)
        const wrapper = buildExportWrapper(clone, title)
      ...
      const addNodeAsPages = async (node, title, addNewPageFirst = false) => {
        const canvas = await renderNodeToCanvas(node, title)
        ...
        while (rendered < canvas.height) {
          ...
          if (addNewPageFirst || pageIndex > 0) pdf.addPage()
      ...
      const addNodeAsSinglePage = async (node, title, addNewPageFirst = false) => {
        const canvas = await renderNodeToCanvas(node, title)
        const imgWidthMm = usableWidth
        const imgHeightMm = (canvas.height * imgWidthMm) / canvas.width
        const fittedWidth = imgHeightMm > usableHeight ? (canvas.width * usableHeight) / canvas.height : imgWidthMm
        const fittedHeight = imgHeightMm > usableHeight ? usableHeight : imgHeightMm
        const offsetX = margin + ((usableWidth - fittedWidth) / 2)
        const offsetY = margin + ((usableHeight - fittedHeight) / 2)
        if (addNewPageFirst) pdf.addPage()
        pdf.addImage(canvas.toDataURL('image/png'), 'PNG', offsetX, offsetY, fittedWidth, fittedHeight)
      }

      if (type === 'risques') {
        const blocks = Array.from(target.querySelectorAll('[data-export-risks-page]'))
        if (blocks.length >= 2) {
          await addNodeAsSinglePage(blocks[0], `${sectionTitles[type]} - Caractéristiques des risques`, false)
          await addNodeAsSinglePage(blocks[1], `${sectionTitles[type]} - Plan de maîtrise`, true)
        } else {
          await addNodeAsPages(target, sectionTitles[type], false)
        }
      } else {
        await addNodeAsPages(target, sectionTitles[type], false)
      }

      pdf.save(`tableau_bord_${type}_${new Date().toISOString().split('T')[0]}.pdf`)
    } catch (error) {
  ...
  const renderRisques = () => {
    ...
    return (
      <div ref={risquesExportRef} className="space-y-4">
        <div className="flex items-center justify-between gap-3 px-1"><h2 className="text-lg font-bold text-gray-800">Suivi des risques</h2><button data-export-exclude="true" onClick={() => exportDashboardSectionToPdf('risques')} className="px-3 py-1.5 text-xs text-gray-700 bg-white border border-gray-200 rounded-lg inline-flex items-center gap-1"><Download size={12}/>PDF</button></div>
        {/* Filtres globaux (identiques à la Synthèse de Gestion des risques) */}
        <div data-export-exclude="true" className="bg-white rounded-lg p-3 shadow-sm border border-gray-200">
        ...
        <div data-export-risks-page="1" className="bg-white rounded-xl p-4 shadow-sm border border-gray-200">
          ...
          <div data-export-exclude="true" className="bg-gray-50 rounded-lg p-3 mb-4 border border-gray-200">
            <div className="mobile-inline-scroll flex flex-wrap md:flex-nowrap items-end gap-2">
        ...
        <div data-export-risks-page="2" className="bg-white rounded-xl p-4 shadow-sm border border-gray-200">
          <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
  ...
  const renderIndicateurs = () => {
    ...
    return (
      <div ref={indicatorsExportRef} className="space-y-4 p-6 pt-4 bg-gray-50 min-h-full">
        <div className="flex items-center justify-between gap-3"><h2 className="text-lg font-bold text-gray-800">Suivi des indicateurs</h2><button data-export-exclude="true" onClick={() => exportDashboardSectionToPdf('indicateurs')} className="px-3 py-1.5 text-xs text-gray-700 bg-white border border-gray-200 rounded-lg inline-flex items-center gap-1"><Download size={12}/>PDF</button></div>
        ...
        <div className="grid grid-cols-5 gap-3 mobile-card-grid-5">
        ...
        <div className="grid grid-cols-2 gap-4 px-1 mobile-card-grid-2">
  ...
  const renderActions = () => {
    ...
    return (
      <div ref={actionsExportRef} className="space-y-4 p-6 pt-4 bg-gray-50 min-h-full">
        <div className="flex items-center justify-between gap-3"><h2 className="text-lg font-bold text-gray-800">Suivi des actions</h2><button data-export-exclude="true" onClick={() => exportDashboardSectionToPdf('actions')} className="px-3 py-1.5 text-xs text-gray-700 bg-white border border-gray-200 rounded-lg inline-flex items-center gap-1"><Download size={12}/>PDF</button></div>
        ...
        <div className="grid grid-cols-4 gap-3 mobile-card-grid-4">
  ...
  return (
    <div className="mobile-subrubric-layout flex h-[calc(100vh-140px)]">
      <div className="mobile-subrubric-sidebar w-56 bg-white border border-gray-200 rounded-2xl p-3 space-y-2 flex-shrink-0 sticky top-0 self-start h-[calc(100vh-140px)] overflow-y-auto">
        <div className="mobile-subrubric-sidebar-grid">{subPages.map((page) => (
          <SidebarButton key={page.key} icon={page.icon} label={page.label} active={activeTab === page.key} onClick={() => handleTabChange(page.key)} />
        ))}</div>
      </div>

      <div className="mobile-subrubric-content flex-1 min-w-0 overflow-auto bg-gray-50 p-2">
        {loading ? (
