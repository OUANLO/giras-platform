...
export default function AdminPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const body = { ...formData, superieur: formData.superieur || null, superieur_existe: formData.superieur ? 'Oui' : 'Non', id: selectedItem?.id, createur: user?.username, modificateur: user?.username }
  ...
  const getSuperieurOptions = () => {
    ...
    const sameStructureUsers = users.filter(u =>
      u.statut === 'Actif' &&
      !!selectedStructure &&
    ...
    const elevatedUsers = users.filter(u =>
      u.statut === 'Actif' &&
      u.username !== currentUsername &&
      (u.type_utilisateur === 'Super admin' || u.type_utilisateur === 'Super manager')
    )

    const unique = new Map()
    ;[...sameStructureUsers, ...elevatedUsers].forEach(u => {
      if (!unique.has(u.username)) {
        unique.set(u.username, {
          value: u.username,
    ...
    if (formData.superieur && !unique.has(formData.superieur)) {
      const current = users.find(u => u.username === formData.superieur)
      if (current && current.username !== currentUsername) {
        unique.set(current.username, {
          value: current.username,
          label: `${current.nom} ${current.prenoms} (${current.username})`
        })
      }
    }
  ...
  const userColumns = [
    ...
    { key: 'structure', label: 'Structure', width: '120px', render: (v) => <span className="text-xs font-mono">{v || '-'}</span> },
    { key: 'superieur', label: 'Supérieur', width: '180px', render: (v) => <span className="text-xs text-gray-600">{v ? getUserDisplayName(v) : '-'}</span> },
          ...
          {modalType === 'user' && (
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2">
                <SearchableSelect label="Supérieur hiérarchique direct" value={formData.superieur || ''} onChange={(v) => setFormData({ ...formData, superieur: v, superieur_existe: v ? 'Oui' : 'Non' })} options={getSuperieurOptions()} placeholder={formData.structure ? 'Sélectionner un supérieur...' : 'Choisir la structure d’abord'} searchPlaceholder="Rechercher un supérieur..." disabled={!formData.structure} />
                <p className="mt-1 text-xs text-gray-500">Champ facultatif. Liste limitée aux utilisateurs actifs de la structure choisie, plus les Super Admin et Super Manager actifs.</p>
              </div>
