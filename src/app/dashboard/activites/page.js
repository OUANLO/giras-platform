...
import { FolderOpen, List, CheckSquare, Plus, Edit, Trash2, Users, PlayCircle, ListChecks, RotateCcw, UserMinus, Eye, CheckCircle, XCircle, Download, Archive } from 'lucide-react'
import { Button, Modal, SidebarButton, StatusBadge, SearchableSelect, AlertModal } from '@/components/ui'
import * as XLSX from 'xlsx'
import { canAccessAction, canAccessActionOccurrence, isPrivilegedUser } from '@/lib/access-scope'
...
Cette action est irréversible.`
  ...
  const fetchActions = async () => {
    try {
      const [r, riskRes] = await Promise.all([fetch('/api/actions'), fetch('/api/actions/risques-suivi')])
  ...
  const fetchOccurrences = async () => {
    try {
      const [r, riskRes] = await Promise.all([fetch('/api/actions/occurrences'), fetch('/api/actions/risques-suivi')])
  ...
  const canViewProjet = (p) => {
    ...
    return actions.some((action) => !action?.archive && getProjectCodeForAction(action) === projectCode && canAccessAction(user, action, users, occurrences))
  }
  ...
  const canDeleteProjet = (p) => p?.code_groupe !== 'RISQUES' && isAdmin()
  const canViewMembres = (p) => isAdmin()
  
  const canViewAction = (a) => {
    const p = getProjectForAction(a)
    if (isGestionnaireProjet(p)) return true
    return canAccessAction(user, a, users, occurrences)
  }
  const canEditAction = (a) => { const p = getProjectForAction(a); return isAdmin() || isGestionnaireProjet(p) }
  const canCreateAction = (p) => isAdmin() || isGestionnaireProjet(p)
  
  const canEditOccurrence = (o) => { const a = actions.find(x => normalizeActionCode(x.code_action) === normalizeActionCode(o?.code_action || o?.code_action_occ || o?.__actionCode)); const p = getProjectForAction(a); return isAdmin() || isGestionnaireProjet(p) }
  const canViewOccurrence = (o) => {
    ...
    const p = getProjectForAction(a)
    if (isGestionnaireProjet(p)) return true
    return canAccessActionOccurrence(user, o, users, actions)
  }
  ...
  const projetOptions = (selectedAction || isAdmin()
    ? allProjetOptions
    : projets
  ...
  const getFilteredActions = () => actions.filter(a => canViewAction(a) && !a.archive && (!actionFilters.projet || getProjectCodeForAction(a) === actionFilters.projet) && (!actionFilters.structure || a.code_structure === actionFilters.structure) && (!actionFilters.statut || (a.statut || a.statut_act) === actionFilters.statut) && (!actionFilters.search || a.libelle_action?.toLowerCase().includes(actionFilters.search.toLowerCase())))
  const getFilteredOccurrences = (sourceOccurrences = occurrences) => sourceOccurrences.filter(o => { 
    if (o.archive) return false
    const a = actions.find(x => normalizeActionCode(x.code_action) === normalizeActionCode(o.code_action || o.code_action_occ || o.__actionCode))
    if (!a || !canViewAction(a) || !canViewOccurrence(o)) return false
    if (suiviFilters.projet && getProjectCodeForAction(a) !== suiviFilters.projet) return false
  ...
  const getFilteredArchivedActions = () => archivedItems.filter(a => a.archive && canViewAction(a) && (!actionFilters.projet || getProjectCodeForAction(a) === actionFilters.projet) && (!actionFilters.structure || a.code_structure === actionFilters.structure) && (!actionFilters.statut || a.statut === actionFilters.statut || a.statut_act === actionFilters.statut) && (!actionFilters.search || a.libelle_action?.toLowerCase().includes(actionFilters.search.toLowerCase())))
  const getFilteredArchivedOccurrences = () => archivedItems.filter(o => {
    ...
    if (suiviFilters.projet && getProjectCodeForAction(linkedAction) !== suiviFilters.projet) return false
    if (suiviFilters.structure && linkedAction.code_structure !== suiviFilters.structure) return false
    if (suiviFilters.responsable && o.responsable !== suiviFilters.responsable) return false
    ...
    return (
      <div className="space-y-4">
        <div className="bg-white rounded-lg p-3 shadow-sm border flex flex-wrap gap-3 items-end">
          ...
          <button onClick={() => setActionFilters({projet:'', structure:'', statut:'', search:''})} className="p-1.5 hover:bg-gray-100 rounded border" title="Réinitialiser"><RotateCcw size={14} className="text-gray-600"/></button>
        </div>
        <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
          <div className="overflow-x-auto overflow-y-auto" style={{maxHeight:'550px'}}>
            <table className="w-full text-[10px]">
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white rounded-lg p-3 shadow-sm border space-y-2">
          ...
          <div className="flex gap-2 items-end">
            ...
            <button onClick={() => handleViewArchives('suivi_action')} className="px-3 py-1.5 text-xs font-medium text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg flex items-center gap-1 self-end"><Archive size={14}/>Archives</button>
          </div>
        </div>
        <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
                ...
                {loading ? <tr><td colSpan={10} className="px-4 py-6 text-center text-gray-500">Chargement...</td></tr> : filtered.length === 0 ? <tr><td colSpan={10} className="px-4 py-6 text-center text-gray-500">Aucune occurrence</td></tr> : filtered.map((o, i) => {
                  ...
                  return (
                    <tr key={o.id} className={`hover:bg-blue-50/50 ${bgColor}`}>
                      <td className={`px-2 py-1.5 sticky left-0 z-10 ${bgColor} border-r max-w-[180px] truncate`} title={o.libelle_action || action?.libelle_action}>{o.libelle_action || action?.libelle_action || '-'}</td>
