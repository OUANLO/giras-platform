...
export default function ProfilPage() {
  ...
  useEffect(() => {
    ...
    const loadStructures = async () => {
      try {
        const res = await fetch('/api/structures?statut=Actif')
        if (!res.ok) return
        const data = await res.json()
        setStructures(data.structures || [])
      } catch (error) {
        console.error('Erreur chargement structures profil:', error)
      }
    }
  ...
  const userStructureCode = user?.structure || user?.code_structure || user?.codeStructure || ''
  const userStructure = structures.find((item) => item.code_structure === userStructureCode)
  const userStructureLabel = userStructureCode
    ? (userStructure?.libelle_structure ? `${userStructureCode} - ${userStructure.libelle_structure}` : userStructureCode)
    : '-'
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-1">
            ...
            <p className="text-lg text-gray-900">{userStructureLabel}</p>
          </div>
          <div className="space-y-1">
