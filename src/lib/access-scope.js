...
export const isSuperManagerUser = (user) => user?.type_utilisateur === 'Super manager'
...
export const canAccessIndicator = (user, indicateur, users = [], structures = []) => {
  ...
  const indicatorStructure = normalizeStructure(indicateur?.code_structure || indicateur?.structure)
  if (isStructureResponsible(user, indicatorStructure, structures)) return true

  return false
}

export const canAccessIndicatorOccurrence = (user, occurrence, indicateurs = [], users = [], structures = []) => {
  if (!user || !occurrence) return false
  if (isPrivilegedUser(user)) return true
  ...
  if (linkedIndicator) return canAccessIndicator(user, linkedIndicator, users, structures)
